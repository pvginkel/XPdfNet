...
{
    const point_t*p1 = (const point_t*)o1;
    const point_t*p2 = (const point_t*)o2;
    return p1->x == p2->x && p1->y == p2->y;
}
static unsigned int point_hash(const void*o)
{
    const point_t*p = (const point_t*)o;
    return p->x^p->y;
}
static void* point_dup(const void*o)
{
    const point_t*p = (const point_t*)o;
    point_t*n = malloc_safe(sizeof(point_t));
    n->x = p->x;
    n->y = p->y;
    return n;
}
...
void gfxpoly_save_arrows(gfxpoly_t*poly, const char*filename)
{
	...
	for(;s!=end;s+=dir) {
	    ...
	    int lx = p.x - o.x;
	    int ly = p.y - o.y;
	    double d = sqrt((double)(lx*lx+ly*ly));
	    if(!d) d=1;
	    else   d = l / d;
	    double d2 = d*1.5;
...
static void segment_init(segment_t*s, int32_t x1, int32_t y1, int32_t x2, int32_t y2, int polygon_nr, segment_dir_t dir)
{
	 ...
	 */
        if(x1>x2) {
            s->dir =auto_cast(DIR_INVERT(s->dir));
...
static hevents_t hevents_fill(status_t*status)
{
    ...
    hevents_t e;
    e.events = malloc_safe(sizeof(hevent_t)*(horiz->num*2 + xrow->num));
    e.num = 0;
...
static void process_horizontals(status_t*status)
{
    ...
    int num_open = 0;
    horizontal_t**open = malloc_safe(sizeof(horizontal_t*)*horiz->num);
...
gfxpoly_t* gfxpoly_process(gfxpoly_t*poly1, gfxpoly_t*poly2, windrule_t*windrule, windcontext_t*context)
{
    ...
    gfxpoly_t*p = (gfxpoly_t*)malloc_safe(sizeof(gfxpoly_t));
