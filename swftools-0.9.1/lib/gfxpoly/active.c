...
static void actlist_splay_dump2(actlist_t*a, segment_t*s, char*mid, char*up, char*down)
{
	...
	if(s->leftchild) {
	    char*o3 = malloc_safe(strlen(up)+3);
	    strcpy(o3, up);strcat(o3, "+-");
	    char*newup = malloc_safe(strlen(up)+3);
	    strcpy(newup, up);strcat(newup, "| ");
	    char*newup2 = malloc_safe(strlen(up)+3);
        ...
        fprintf(stderr, "%s[%d]\n", mid, SEGNR(s));
	if(s->rightchild) {
	    char*o3 = malloc_safe(strlen(down)+3);
	    strcpy(o3, down);strcat(o3, "+-");
	    char*newdown = malloc_safe(strlen(down)+3);
	    strcpy(newdown, down);strcat(newdown, "| ");
	    char*newdown2 = malloc_safe(strlen(down)+3);
