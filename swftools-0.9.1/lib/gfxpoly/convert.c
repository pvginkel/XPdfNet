...
void finish_segment(compactpoly_t*data)
{
	...
	return;
    point_t*p = malloc_safe(sizeof(point_t)*data->num_points);
...
void gfxpolywriter_init(polywriter_t*w)
{
    ...
    compactpoly_t*data = rfx_calloc(sizeof(compactpoly_t));
	w->internal = data;
...
gfxline_t*gfxline_from_gfxpoly(gfxpoly_t*poly)
{
    ...
    if(!count) return 0;
    gfxline_t*l = malloc_safe(sizeof(gfxline_t)*count);
    count = 0;
...
static gfxline_t*mkgfxline(gfxpoly_t*poly, char preserve_direction)
{
	...
	stroke = auto_cast(dict_lookup(d, &last));
	should_connect = 1;
