...
static inline void posqueue_addpos(posqueue_t*queue, node_t*pos)
{
    ...
    queue->list =(posqueue_entry_t*) malloc(sizeof(posqueue_entry_t));
    queue->list->pos = pos;
    queue->list->next = old;
}
...
{
    graph_t*graph = (graph_t*)rfx_calloc(sizeof(graph_t));
    graph->num_nodes = num_nodes;
    graph->nodes = (node_t*)rfx_calloc(sizeof(node_t)*num_nodes);
    int t;
...
{
    graphcut_workspace_t*workspace = (graphcut_workspace_t*)malloc(sizeof(graphcut_workspace_t));
    workspace->flags1 = (unsigned char*)rfx_calloc(graph->num_nodes);
    workspace->flags2 = (unsigned char*)rfx_calloc(graph->num_nodes);
    workspace->back = (halfedge_t**)rfx_calloc(graph->num_nodes*sizeof(halfedge_t*));
    workspace->pos1 = pos1;
    workspace->pos2 = pos2;
...
static path_t*path_new(int len)
{
    ...
    p->pos = (node_t**)malloc(sizeof(node_t*)*len);
    p->dir = (halfedge_t**)malloc(sizeof(halfedge_t*)*len);
    p->firsthalf =(unsigned char*)malloc(sizeof(unsigned char)*len);
    p->length = len;
    return p;
}
