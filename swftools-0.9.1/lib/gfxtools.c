...
gfxline_t*gfxline_makecircle(double x,double y,double rx, double ry)
{
    ...
    int t;
    for(t=0;t<9;t++) {
	line[t] = (gfxline_t*)rfx_calloc(sizeof(gfxline_t));
    }
...
gfxbbox_t* gfxline_isrectangle(gfxline_t*_l)
{
    ...
    gfxbbox_t*r = (gfxbbox_t*)malloc(sizeof(gfxbbox_t));
    r->xmin = x1; r->ymin = y1;
    r->xmax = x2; r->ymax = y2;
    return r;
}
...
{
    return string_hash3((const char *)c, sizeof(gfxpoint_t));
}
...
gfxline_t* gfxline_restitch(gfxline_t*line)
{
	...
	gfxline_t*l =(gfxline_t*) dict_lookup(ff, &pos);
	if(l) {
	    char d = dict_del2(ff,&pos,l);assert(d);
	} else {
	    l = (gfxline_t*)dict_lookup(rev, &pos);
	    if(l) {
		reverse = 1;
