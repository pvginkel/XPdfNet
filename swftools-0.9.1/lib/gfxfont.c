...
void gfxfont_fix_unicode(gfxfont_t*font)
{
    ...
    char*used = (char*)rfx_calloc(max+1);
...
{ 
	...
	font->unicode2glyph = (int*)malloc(sizeof(font->unicode2glyph[0])*(font->max_unicode));
	...
	if(font->max_unicode < new_max_unicode) {
	    font->unicode2glyph =(int*) rfx_realloc(font->unicode2glyph, sizeof(font->unicode2glyph[0])*(font->max_unicode));
...
ttf_t* gfxfont_to_ttf(gfxfont_t*font, char eot)
{
    ...
    ttf->glyphs = (ttfglyph_t*)rfx_calloc(num_glyphs*sizeof(ttfglyph_t));
    double scale = 1.0;
	...
	dest->points = (ttfpoint_t*)rfx_calloc(count*sizeof(ttfpoint_t));
	count = 0;
	line = src->line;
	while(line) {
	    if(line->type == gfx_splineTo) {
    ...
    ttf->unicode = (unicode_t*)rfx_calloc(sizeof(unicode_t)*ttf->unicode_size);
