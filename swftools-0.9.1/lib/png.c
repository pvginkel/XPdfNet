...
static int png_read_chunk(char (*head)[4], int*destlen, unsigned char**destdata, FILE*fi)
{
	...
	if(!len) {
	    ...
	    *destdata = (unsigned char*)malloc_safe(len);
...
EXPORT int getPNG(const char*sname, int*destwidth, int*destheight, unsigned char**destdata)
{
    ...
    imagedata = (unsigned char*)malloc_safe(imagedatalen);
	...
	if(!strncmp(tagid, "IDAT", 4)) {
	    if(!zimagedata) {
		zimagedatalen = len;
		zimagedata = (unsigned char*)malloc_safe(len);
    ...
    *destheight = header.height;
    ...
    data2 = (unsigned char*)malloc_safe(header.width*header.height*4);
	...
	int x,y;
	int pos=0;
	unsigned char* old= (unsigned char*)malloc_safe(header.width*2);
	...
	*destdata = data2;
	...
	unsigned char* firstline = malloc_safe(header.width*4);
	...
	COL*rgba = 0;
	unsigned char*tmpline = (unsigned char*)malloc_safe(header.width+1);
	unsigned char*destline = (unsigned char*)malloc_safe(header.width+1);
	int i,x,y;
	int pos=0;
	...
	*destdata = data2;
	...
	if(header.mode == 0) { // grayscale palette
	    ...
	    rgba = (COL*)malloc_safe(palettelen*sizeof(COL));
	    ...
	    rgba = (COL*)malloc_safe(palettelen*4);
...
{
    unsigned char*colexists = malloc_safe((256*256*256)/8);
    ...
    colornum_t*colors=(colornum_t*)malloc_safe(sizeof(colornum_t)*count);
    int pos=0;
...
static void getOptimalPalette(COL*image, int size, int palettesize, COL*palette)
{
    ...
    colornum_t*centers = malloc_safe(sizeof(colornum_t)*palettesize);
    int t;
    for(t=0;t<palettesize;t++) {
        centers[t].color = colors[t].color;
    }
    unsigned char*belongsto = (unsigned char*)malloc_safe(num);
    ...
    while(change) {
        ...
        for(t=0;t<num;t++) {
            ...
            for(s=0;s<palettesize;s++) {
                ...
                distance += abs((long)((centers[s].color>>0&0xff) - (colors[t].color>>0&0xff)));
                distance += abs((long)((centers[s].color>>8&0xff) - (colors[t].color>>8&0xff)));
                distance += abs((long)((centers[s].color>>16&0xff) - (colors[t].color>>16&0xff)));
...
static void png_quantize_image(unsigned char*_image, int size, int numcolors, unsigned char**newimage, COL*palette) 
{
    ...
    *newimage = (unsigned char*)malloc_safe(size);
    int t;
    for(t=0;t<size;t++) {
        int best=0x7fffffff;
        int bestcol = 0;
...
static void make_crc32_table(void)
{
  ...
  if(crc32_table) 
      return;
  crc32_table = (u32*)malloc_safe(1024);
...
static int png_get_number_of_palette_entries(COL*img, int width, int height, COL*palette, char*has_alpha)
{
    ...
    u32*pal = (u32*)malloc_safe(65536*sizeof(u32));
    int*count = (int*)malloc_safe(65536*sizeof(int));
...
static void make_num_bits_table()
{
    ...
    num_bits_table = malloc_safe(sizeof(num_bits_table[0])*256);
    int t;
    for(t=0;t<256;t++) {
	int bits=0;
	int v = t;
...
static int png_find_best_filter(unsigned char*src, int width, int bpp, int y)
{
    ...
    pairs[0] = calloc_safe(1, 8192);
    pairs[1] = calloc_safe(1, 8192);
    pairs[2] = calloc_safe(1, 8192);
    pairs[3] = calloc_safe(1, 8192);
    pairs[4] = calloc_safe(1, 8192);
...
static int png_apply_filter(unsigned char*dest, unsigned char*src, int width, int y, int bpp)
{
    ...
    int best_energy = INT_MAX;
    int w = width*(bpp/8);
    unsigned char* pairs = malloc_safe(8192);
    assert(bpp==8 || bpp==32);
    for(f=0;f<num_filters;f++) {
	if(bpp==8)
...
EXPORT void savePNG(const char*filename, unsigned char*data, int width, int height, int numcolors)
{
	...
	if(num<=255) {
	    ...
	    data2 = malloc_safe(width*height);
    ...
    Bytef*writebuf = (Bytef*)malloc_safe(ZLIB_BUFFER_SIZE);
    zs.zalloc = Z_NULL;
    zs.zfree  = Z_NULL;
    zs.opaque = Z_NULL;
	...
	unsigned char* line = (unsigned char*)malloc_safe(linelen);
	memset(line, 0, linelen);
#if 0
	unsigned char* bestline = (unsigned char*)malloc_safe(linelen);
