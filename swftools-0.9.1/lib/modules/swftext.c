...
int swf_FontExtract_DefineFontAlignZones(int id, SWFFONT * font, TAG * tag)
{
	...
	font->alignzones =(ALIGNZONE*) rfx_calloc(sizeof(ALIGNZONE)*font->numchars);
	int i=0;
...
void swf_FontUsePair(SWFFONT * f, int char1, int char2)
{
	...
	u->neighbors_hash = (int*)rfx_calloc(u->neighbors_hash_size*sizeof(int));
	int t;
	...
	if(u->num_neighbors == u->neighbors_size) {
	    ...
	    u->neighbors = (SWFGLYPHPAIR*)rfx_realloc(u->neighbors, sizeof(SWFGLYPHPAIR)*u->neighbors_size);
	}
...
void swf_WriteFont_AS3(SWFFONT * font, char *filename)
{
    ...
    char*name =(char*)(font->name?(char*)font->name:"font");
...
void swf_WriteFont(SWFFONT * font, char *filename)
{
	...
	double width = 0;
	U16 max_advance = 0;
	char*flags = (char*)rfx_calloc(font->numchars);
	double*xmin =(double*) rfx_calloc(sizeof(double)*(font->numchars+1));
	double*xmax = (double*)rfx_calloc(sizeof(double)*(font->numchars+1));
	int*xpos = (int*)rfx_calloc(sizeof(int)*(font->numchars+1));
	for(t=0;t<font->numchars;t++) {
