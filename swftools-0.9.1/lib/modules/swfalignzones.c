...
static void find_best(float*_row, int width, int*_x1, int*_x2, int min_size, int from, int to, int num, char debug)
{
    ...
    int t;
    float*row = (float*)malloc(sizeof(float)*(width+1));
    int filter_size = 10;
    float* filter = (float*)malloc(sizeof(float)*(filter_size*2+1));
...
void swf_FontCreateAlignZones(SWFFONT * f)
{
	...
	float*row = (float*)rfx_calloc(sizeof(float)*(width+1));
	float*column_global = (float*)rfx_calloc(sizeof(float)*(height+1));
	float*column = (float*)rfx_calloc(sizeof(float)*(height+1));
