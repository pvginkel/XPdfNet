...
static void* mymatrix_clone(const void*_m) {
    if(_m==0) 
        return 0;
    const mymatrix_t*m1=(const mymatrix_t*)_m;
    mymatrix_t*m2 = (mymatrix_t*)malloc(sizeof(mymatrix_t));
    *m2 = *m1;
    m2->id = strdup(m1->id);
    return m2;
}
...
static char mymatrix_equals(const void*_m1, const void*_m2) {
    const mymatrix_t*m1=(const mymatrix_t*)_m1;
    const mymatrix_t*m2=(const mymatrix_t*)_m2;
    if(!m1 || !m2) 
        return m1==m2;
...
{
    transformedfont_t*f = (transformedfont_t*)rfx_calloc(sizeof(transformedfont_t));
    f->orig = orig;
    f->matrix = *m;
    f->used =(int*) rfx_calloc(sizeof(f->used[0])*orig->num_glyphs);
    int t;
...
static void pass1_drawchar(gfxfilter_t*f, gfxfont_t*font, int glyphnr, gfxcolor_t*color, gfxmatrix_t*matrix, gfxdevice_t*out)
{
	...
	return;
    transformedfont_t*fd = (transformedfont_t*)dict_lookup(i->matrices, &m);
    if(!fd) {
...
static gfxresult_t* pass1_finish(gfxfilter_t*f, gfxdevice_t*out)
{
	...
	gfxfont_t*font = fd->font = (gfxfont_t*)rfx_calloc(sizeof(gfxfont_t));
	char id[80];
	...
	if(fd->matrix.m00>0) {
	    ...
	    font->kerning =(gfxkerning_t*) malloc(sizeof(font->kerning[0])*count);
...
static void pass2_drawchar(gfxfilter_t*f, gfxfont_t*font, int glyphnr, gfxcolor_t*_color, gfxmatrix_t*matrix, gfxdevice_t*out)
{
    ...
    transformedfont_t*d =(transformedfont_t*) dict_lookup(i->matrices, &m);
