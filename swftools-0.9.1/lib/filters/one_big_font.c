...
static void pass1_drawchar(gfxfilter_t*f, gfxfont_t*font, int glyphnr, gfxcolor_t*color, gfxmatrix_t*matrix, gfxdevice_t*out)
{
    ...
    fontdata_t*d =(fontdata_t*) gfxfontlist_getuserdata(i->fonts, font->id);
    if(!d) {
	d = (fontdata_t*)rfx_calloc(sizeof(fontdata_t));
	d->font = font;
...
{
    internal_t*i = (internal_t*)f->internal;
    gfxfont_t*font = i->font = (gfxfont_t*)rfx_calloc(sizeof(gfxfont_t));
    font->id = strdup("onebigfont");
    font->num_glyphs = i->num_glyphs;
    font->glyphs =(gfxglyph_t*) rfx_calloc(sizeof(gfxglyph_t)*i->num_glyphs);
    gfxfontlist_t*l = i->fonts;
    while(l) {
	gfxfont_t*old = l->font;
	fontdata_t*d = (fontdata_t*)l->user;
