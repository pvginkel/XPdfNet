...
static gfxdocument_t*pdf_open(gfxsource_t*src, const char*filename)
{
	...
	*x = 0;
	...
	if(!global_page_range || is_in_range(t, global_page_range)) {
		...
		if(i->info->x1 < 0 && i->info->y1 < 0 )
		{
			i->info->x1 = i->info->y1 = 0;
			i->info->x2 = i->doc->getPageMediaWidth(t);
			i->info->y2 = i->doc->getPageMediaHeight(t);
		}
...
#ifndef _CONSOLE
static gfxdocument_t*pdf_open_stream(gfxsource_t*src, void *stream, void *pdfDoc)
{
	gfxsource_internal_t*isrc = (gfxsource_internal_t*)src->internal;
    gfxdocument_t*pdf_doc = (gfxdocument_t*)malloc(sizeof(gfxdocument_t));
    memset(pdf_doc, 0, sizeof(gfxdocument_t));
    ...
    i->filename = new char[8];
	sprintf_s(i->filename, 8, "stream");
	...
	else
	{
		...
		i->doc = new PDFDoc(static_cast<BaseStream *>(stream), userPW);
		if (userPW) 
		  delete userPW;
	}
...
gfxsource_t*gfxsource_pdf_create()
{
    ...
    src->open = pdf_open;
#ifndef _CONSOLE
	src->open_stream = pdf_open_stream;
#endif
