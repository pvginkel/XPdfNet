...
gfxfilterchain_t* gfxfilterchain_parse(const char*_filterexpr)
{
	...
	*colon = 0;
	...
	if(!param || lastitem) {
		...
		char*alphastr = (char *) dict_lookup(params, "alpha");
		int alpha = 255;
		if(alphastr) alpha=atoi(alphastr);
		f = (gfxfilterbase_t *)malloc(sizeof(gfxfilter_t));
		gfxfilter_maketransparent_init((gfxfilter_t*)f, alpha);
	    } else if(!strcmp(cmd, "remove_font_transforms")) {
		f = (gfxfilterbase_t *)malloc(sizeof(gfxtwopassfilter_t));
	    ...
	    dict_clear(params);
	    gfxfilterchain_t*n = (gfxfilterchain_t*)rfx_calloc(sizeof(gfxfilterchain_t));
	    if(!chain) {
		chain = next = n;
	    } else {
		next->next = n;
