...
{
    const double*y1 = (const double *)_y1;
    const double*y2 = (const double *)_y2;
    if(*y1<*y2) return -1;
    if(*y1>*y2) return 1;
    else return 0;
}
...
int check_svp(ArtSVP*svp)
{
    ...
    /* create segs and ys */
    double*y = malloc_safe(sizeof(double)*num_points);
    svp_segment_part_t*segs = malloc_safe(sizeof(svp_segment_part_t)*num_segs);
    svp_segment_part_t**seg_start = malloc_safe(sizeof(svp_segment_part_t*)*num_segs);
    svp_segment_part_t**seg_end = malloc_safe(sizeof(svp_segment_part_t*)*num_segs);
...
static void my_perturb(ArtVpath*vpath)
{
    ...
    int*pos = (int*)malloc_safe(len*sizeof(int));
    for(t=0;t<len;t++)
        pos[t] = t;
    pvpath = vpath;
