...
extern int args_callback_option_combine(char*,char*);
extern int args_callback_longoption_combine(char*,char*);
extern int args_callback_command_combine(char*,char*);
extern void args_callback_usage_combine(char*name);
...
static void processargs(int argn2,char**argv2)
{
    ...
    if(argn2==1)
    {
        args_callback_usage_combine(argv2[0]);
        exit(1);
    }
    for(t=1;t<argn2;t++)
    {
        ...
        if(argv2[t][0]=='-' && argv2[t][1])
        {
            if(argv2[t][1]=='-')
            {
                ...
                t+=args_callback_longoption_combine(&argv2[t][2],next);
            }
            else
            {
                ...
                if(argv2[t][1]) // this might be something like e.g. -xvf
                {
                    ...
                    do{
                        if(argv2[t][s+1]) {
                          buf[0] = argv2[t][s];
                          ret = args_callback_option_combine(buf,&argv2[t][s+1]);
                        }
                        else {
                          t+= args_callback_option_combine(&argv2[t][s], next);
                          break;
                        }
                        s++;
                    } while(!ret);
        ...
        {
	    int num = args_callback_command_combine(argv2[t],next);
	    if(num>2) {
    ...
    while(options->shortoption) {
        if(!strcmp(options->longoption, name)) {
                ...
                if(equal) {
                    ...
                    int ret = args_callback_option_combine(tmp, equal);
                    if(!ret) {
                        fprintf(stderr, "Warning: Option --%s takes no parameter.\n", name);
                    }
                    return 0;
                }
                return args_callback_option_combine(tmp,val);
        }
        options++;
    }
