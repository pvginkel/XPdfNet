...
heap_t* heap_clone(heap_t*o)
{
    ...
    h->elements =(void**)rfx_alloc(sizeof(void*)*h->size);
    int t;
    for(t=0;t<h->size;t++) {
...
{
    if(!*t) {
        (*t) = (trielayer_t*)rfx_calloc(sizeof(trielayer_t));
...
string_t* string_new3(const char*text, int len)
{
    if(!text) {
        ...
        string_t*s = (string_t*)malloc(sizeof(string_t)+len+1);
        s->len = len;
...
void stringarray_init(stringarray_t*sa, int hashsize)
{
    ...
    mem_init(&s->pos);
    s->hash = (stringlist_t**)rfx_calloc(sizeof(stringlist_t*)*hashsize);
    s->hashsize = hashsize;
}
void stringarray_put(stringarray_t*sa, string_t str)
{
    ...
    stringlist_t*l = (stringlist_t*)rfx_alloc(sizeof(stringlist_t));
    l->index = s->num;
    l->next = s->hash[hash];
    s->hash[hash] = l;
...
unsigned int charptr_hash(const void*o) 
{
    if(!o)
        return 0;
    return string_hash2((const char*)o);
}
void* charptr_dup(const void*o) 
{
    if(!o)
        return 0;
    return strdup((const char*)o);
}
...
{
    if(!o) return 0;
    return string_hash((const string_t*)o);
}
string_t*string_dup3(string_t*o)
{
    ...
    string_t*s =(string_t*) rfx_alloc(sizeof(string_t)+o->len+1);
    s->len = o->len;
    s->str = (const char*)(s+1);
...
dict_t*dict_new()
{
    dict_t*d = (dict_t*)rfx_alloc(sizeof(dict_t));
...
{
    dict_t*h =(dict_t*) rfx_alloc(sizeof(dict_t));
...
{
    map_internal_t*m = (map_internal_t*)map->internal;
    const char*value =(const char*) dict_lookup(&m->d, name);
    return value;
}
...
array_t* array_new() {
    array_t*d = (array_t*)malloc(sizeof(array_t));
...
int array_append(array_t*array, const void*name, void*data) {
	...
	array->size += 64;
	if(!array->d) {
	    array->d =(array_entry_t*) malloc(sizeof(array_entry_t)*array->size);
	} else {
	    array->d =(array_entry_t*) realloc(array->d, sizeof(array_entry_t)*array->size);
	}
    }
...
void list_append_(void*_list, void*entry)
{
    ...
    if(!*list) {
        ...
        *list = n;
        ...
        n = (commonlist_t*)malloc(sizeof(commonlist_t));
