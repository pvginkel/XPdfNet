...
static charbuffer_t*charbuffer_append(charbuffer_t*buf, SWFFONT*font, int charid, int x,int y, int size, RGBA color, MATRIX*m)
{
	...
	charbuffer_t*n = (charbuffer_t*)rfx_calloc(sizeof(charbuffer_t));
	n->matrix = *m;
	n->next = buf;
	buf = n;
    }
    if(!buf->last || buf->last->pos == CHARDATAMAX) {
	chararray_t*n = (chararray_t*)rfx_calloc(sizeof(chararray_t));
	if(!buf->array) {
...
static void drawlink(gfxdevice_t*dev, ActionTAG*actions1, ActionTAG*actions2, gfxline_t*points, char mouseover, char*type, const char*url)
{
    ...
    const char* name = 0;
    if(i->config_linknameurl) {
        buf2 = (char*)malloc(strlen(type)+strlen(url)+2);
	...
	MATRIX m;
        ...
        m.tx = p.x;
        m.ty = p.y;
	swf_ObjectPlace(i->tag, buttonid, getNewDepth(dev),&m,0,(const char*)name);
    } else {
	swf_ObjectPlace(i->tag, buttonid, getNewDepth(dev),&i->page_matrix,0,(const char*)name);
    }
...
static GRADIENT* gfxgradient_to_GRADIENT(gfxgradient_t*gradient)
{
    ...
    GRADIENT* swfgradient = (GRADIENT*)malloc(sizeof(GRADIENT));
    swfgradient->num = num;
    swfgradient->rgba = (RGBA*)malloc(sizeof(swfgradient->rgba[0])*num);
    swfgradient->ratios = (U8*)malloc(sizeof(swfgradient->ratios[0])*num);
