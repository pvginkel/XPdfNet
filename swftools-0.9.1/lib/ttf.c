...
static void readBlock(memreader_t*r, void*dest, int len)
{
	...
	memset((void*)((int)dest+remaining), 0, len - remaining);
	r->pos += remaining;
    } else {
...
static void expand(ttf_table_t*w, int newsize)
{
    ...
    w->data = (U8*)rfx_realloc(w->data, w->memsize);
}
...
{
    table_head_t*head = (table_head_t*)rfx_calloc(sizeof(table_head_t));
...
{
    ttf->head = (table_head_t*)rfx_calloc(sizeof(table_head_t));
...
static int parse_simple_glyph(ttf_t*ttf, memreader_t*r, int num_contours, int glyphnr)
{
    ...
    U16*endpoints = 0;
    if(num_contours>0) {
	endpoints = malloc_safe(sizeof(U16)*num_contours);
	int s;
	int lastpos = -1;
    ...
    if(code_len) {
	glyph->code = malloc_safe(sizeof(U16)*code_len);
...
{
    U32*locations = malloc_safe(sizeof(U32)*(ttf->num_glyphs+1));
    int t;
    for(t=0;t<ttf->num_glyphs;t++) {
	locations[t] = w->len;
...
void cmap_parse(memreader_t*r, ttf_t*ttf)
{
	...
	if(format == 0) {
	    ...
	    data = malloc_safe(num*sizeof(unicode_t));
	    int s;
...
void cvt_parse(memreader_t*r, ttf_t*ttf)
{
    ...
    cvt->num = r->size/2;
    cvt->values = malloc_safe(cvt->num*sizeof(S16));
    int t;
    for(t=0;t<cvt->num;t++) {
...
void gasp_parse(memreader_t*r, ttf_t*ttf)
{
    ...
    int t;
    if(!num) return;
    gasp->records = malloc_safe(sizeof(gasp->records[0])*num);
    for(t=0;t<num;t++) {
...
ttf_t* ttf_load(void*data, int length)
{
    ...
    U32*table_data = malloc_safe(16*num_tables);
    int t;
